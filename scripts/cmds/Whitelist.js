  ...
  onStart: async function ({ message, args, usersData, event }) {
    ...
    switch (action) {
      ...
      case "a":
        ...
        if (addIds.length === 0) return message.reply("⚠️ Please provide user ID(s) or tag user(s) to add.");
      ...
      case "r":
        ...
        if (removeIds.length === 0) return message.reply("⚠️ Please provide user ID(s) or tag user(s) to remove.");
      ...
      default:
        return message.reply("⚠️ Invalid input. Use 'add', 'remove', 'list', 'on', or 'off'.");
    }
  }
};
